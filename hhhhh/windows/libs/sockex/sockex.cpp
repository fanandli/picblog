...
SockExTCP::SockExTCP(SOCKET s, SOCKADDR_IN *saddr) :SockEx(s){
  listen = NULL;

  if (s == INVALID_SOCKET) {
    sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    new ((SockExTCP*)this)SockExTCP(sock, saddr); //must use SockExTCP to renew, orelse call virtual deconstruct function, ~SockExTCP cannot be triggered
    return;
  }
  ...
  if (saddr != NULL) {
    ...
    ::bind(sock, (sockaddr*)(saddr), sizeof(sockaddr_in)); // http://www.cnblogs.com/MRRAOBX/articles/3082997.html, must bind, orelse connectex occur 10022 error;
  }
};
...
int SockExTCP::onConnect() {
  if (listen == nullptr) {
    ...
    setsockopt(sock, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char*)&listen->sock, sizeof(listen->sock)); //MUST set this option, orelse cannot use SHUTDOWN, getpeername ...
  }
...
//1, remote socket shutdown 2, local tcp enable keepalive option, and keepalive detect the tcp not working
    ...
    if (op == SockExOL::CONNECT) {
      ...
      ((SockExTCP*)sockex)->AcceptEx(((SockExTCP*)sockex)->listen);
      ((SockExTCP*)sockex)->onConnect();

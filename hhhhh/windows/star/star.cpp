...
void SerializeTlv(StarTlv& tlv) {
  ...
  int n = sendto(g_attr.shellsock->sock, msg.get_final(), msg.total, MSG_NOSIGNAL, (SOCKADDR*)&sockdownaddr, sizeof(sockdownaddr));
  LOG("seriaze sock:%d,n=%d, errid:%d", g_attr.shellsock->sock, n, WSAGetLastError());
}
...
int proInnerNetTypeTest(SockEx* esock, StarTlv& tlvs) {
  ...
  if (g_attr.netType == NBS_NETTYPE::INVALID) {
    ...
    int n = sendto(g_attr.shellsock->sock, msg.get_final(), msg.total, MSG_NOSIGNAL, (SOCKADDR*)&rmtaddr, sizeof(rmtaddr));
    DBG("GET GETSELFADDR:%d, err:%d", n, WSAGetLastError);
...
int Client::proInnerShellMsg(SockEx* esock, StarTlv& tlvs) {
  ...
  if (synheader == nullptr) { // from browser, using udp and no 
    sendto(esock->sock, replystr, strlen(replystr), MSG_NOSIGNAL, (SOCKADDR*)&udppeer, sizeof(SOCKADDR_IN));
  } else {
...
#if 0
int Client::getActiveip(int family) {
  DBG("getActiveip, family:%d", family);
  int sock = -1;
  ...
  try {
    ...
    int len = 0;
    sock = socket(PF_NETLINK, SOCK_DGRAM | SOCK_CLOEXEC, NETLINK_ROUTE);
    if (-1 == sock) {
    ...
    ndp->ndm_family = family;

    if (send(sock, nlMsg, nlMsg->nlmsg_len, MSG_NOSIGNAL) < 0) {
      throw std::logic_error("cannot send");
    }

    while (true) {
      if ((len = recv(sock, msgBuf, 0, MSG_PEEK | MSG_TRUNC)) < 0) {
        throw std::logic_error("recv err");
      }
      rcvmsg = (struct nlmsghdr*)malloc(len);
      if ((len = recv(sock, rcvmsg, len, 0)) < 0) {
        throw std::logic_error("recv err");
      }
        ...
        if (ndp->ndm_state == NUD_REACHABLE) {
          ...
          for (; RTA_OK(ndap, ndl); ndap = RTA_NEXT(ndap, ndl)) {
            if (ndap->rta_type == NDA_DST) {
              ...
              xmac = (char*)RTA_DATA(ndap);
              if (memcmp(xmac, this->mac, IFHWADDRLEN) != 0) {
                break;
              }
            }
            ...
            if (addr != nullptr && mac != nullptr) {
              ...
              //printf("getactive, ip:%s, state:%d\r\n", inet_ntop(ndp->ndm_family, addr, addrstr, INET6_ADDRSTRLEN), ndp->ndm_state);
              //proNeighChange(ndp->ndm_family, addr, RTM_GETNEIGH);
              break;
            }
          }
        }
